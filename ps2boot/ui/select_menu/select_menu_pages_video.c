...
void select_menu_pages_draw_display_page(const select_menu_view_state_t *view_state)
{
    ...
    if (view_state) {
        x = view_state->display_x;
        y = view_state->display_y;
    }
    ...
    select_menu_font_draw_string_color(
        SELECT_MENU_DISPLAY_TITLE_X,
        SELECT_MENU_TITLE_Y,
        "DISPLAY POSITION",
        SELECT_MENU_COLOR_TEXT);

    select_menu_font_draw_string_color(
        SELECT_MENU_DISPLAY_INFO_X,
        SELECT_MENU_DISPLAY_INFO_Y,
        "D-PAD MOVES OUTPUT",
        SELECT_MENU_COLOR_TEXT);

    select_menu_font_draw_string_color(
        SELECT_MENU_DISPLAY_X_X,
        SELECT_MENU_DISPLAY_X_Y,
        buf_x,
        SELECT_MENU_COLOR_HIGHLIGHT);

    select_menu_font_draw_string_color(
        SELECT_MENU_DISPLAY_Y_X,
        SELECT_MENU_DISPLAY_Y_Y,
        buf_y,
        SELECT_MENU_COLOR_HIGHLIGHT);

    select_menu_font_draw_string_color(
        SELECT_MENU_DISPLAY_HINT_X,
        SELECT_MENU_DISPLAY_HINT_Y,
        "CROSS START CIRCLE = BACK",
        SELECT_MENU_COLOR_TEXT);

    select_menu_font_draw_string_color(
        8,
        SELECT_MENU_HINT_BOTTOM_Y,
        "SELECT CLOSE",
        SELECT_MENU_COLOR_TEXT);
}
